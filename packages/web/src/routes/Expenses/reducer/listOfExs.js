...
const defaultState = {
    ...
    totalPages: Math.floor(estimateRecord / defaultPerPage) + (estimateRecord % defaultPerPage === 0 ? 0 : 1),
...
export default createReducer(defaultState, {
    ...
    'Reducer - exs: save total records': (state, { payload: { totalRecords, totalPages, perPage } }) => {
        return {
            ...state,
    ...
    'Reducer: update expense filtering': (state, { payload }) => ({
        ...state,
        ...payload,
    }),
    ...
    'Reducer - exs: set loading off': (state) => ({ ...state, fetching: false }),

    'Reducer - exs: set fetching on': (state) => ({ ...state, fetching: true }),

    'Reducer - exs: reset data to default': (state) => ({
        ...defaultState,
    }),
